...
class GenreTest extends TestCase
{
    ...
    {
        $this->mock(GenreService::class, function (MockInterface $mock) {
            $mock->shouldReceive('getList')
                ->once()
                ->andReturn(new LengthAwarePaginator([], 0, 20));
        });

        $this->withToken($this->token())->getJson('/api/genres')
            ->assertStatus(200)
            ->assertJsonStructure([
                'code', 'message',
                'data' => [
    ...
    {
        $this->withToken($this->token())->getJson('/api/genres?sort=name')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }
    ...
    {
        $this->withToken($this->token())->getJson('/api/genres?order=random')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }

    public function test_index_invalid_type_returns_422(): void
    {
        $this->withToken($this->token())->getJson('/api/genres?type=anime')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }
    ...
    {
        $this->withToken($this->token())->getJson('/api/genres?per_page=101')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }
    ...
    public function test_all_returns_flat_array(): void
    {
        ...
        $this->withToken($this->token())->getJson('/api/genres/all')
            ->assertStatus(200)
