<?php

declare(strict_types=1);

namespace Tests\Feature\Refs;

use App\Models\User;
use App\Services\LanguageService;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Pagination\LengthAwarePaginator;
use Mockery\MockInterface;
use Tests\TestCase;

class LanguageTest extends TestCase
{
    use RefreshDatabase;

    private function token(): string
    {
        return auth('api')->login(User::factory()->create());
    }

    public function test_unauthenticated_request_returns_401(): void
    {
        $this->getJson('/api/languages')
            ->assertStatus(200)
            ->assertJson(['code' => 401]);
    }

    public function test_index_returns_paginated_list(): void
    {
        $this->mock(LanguageService::class, function (MockInterface $mock) {
            $mock->shouldReceive('getList')
                ->once()
                ->andReturn(new LengthAwarePaginator([], 0, 20));
        });

        $this->withToken($this->token())->getJson('/api/languages')
            ->assertStatus(200)
            ->assertJsonStructure([
                'code', 'message',
                'data' => [
                    'list',
                    'pagination' => ['total', 'page', 'per_page', 'last_page'],
                ],
            ])
            ->assertJson(['code' => 0]);
    }

    public function test_index_valid_sort_by_iso_639_1(): void
    {
        $this->mock(LanguageService::class, function (MockInterface $mock) {
            $mock->shouldReceive('getList')
                ->once()
                ->andReturn(new LengthAwarePaginator([], 0, 20));
        });

        $this->withToken($this->token())->getJson('/api/languages?sort=iso_639_1&order=asc')
            ->assertStatus(200)
            ->assertJson(['code' => 0]);
    }

    public function test_index_invalid_sort_returns_422(): void
    {
        $this->withToken($this->token())->getJson('/api/languages?sort=name')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }

    public function test_index_per_page_exceeds_max_returns_422(): void
    {
        $this->withToken($this->token())->getJson('/api/languages?per_page=101')
            ->assertStatus(200)
            ->assertJson(['code' => 422]);
    }

    public function test_all_returns_flat_array(): void
    {
        $this->mock(LanguageService::class, function (MockInterface $mock) {
            $mock->shouldReceive('getAll')
                ->once()
                ->andReturn(new Collection);
        });

        $this->withToken($this->token())->getJson('/api/languages/all')
            ->assertStatus(200)
            ->assertJson(['code' => 0]);
    }

    public function test_all_unauthenticated_returns_401(): void
    {
        $this->getJson('/api/languages/all')
            ->assertStatus(200)
            ->assertJson(['code' => 401]);
    }
}
