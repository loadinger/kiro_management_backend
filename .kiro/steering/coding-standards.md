...
- 如果某个功能在需求文档（spec）中没有明确要求，就不要实现
- 宁可少做，不要多做，功能可以后续通过新的 spec 添加
- 遇到"这个功能以后可能会用到"的想法时，选择不做
- 代码中不要预留"扩展点"或"插件机制"，除非 spec 明确要求
- 如果 AI 认为某个功能对项目很重要但用户没有提到，只能建议，不能直接实现
- 如果用户要求模糊的功能，必须先澄清具体需求和范围
- 如果技术实现需要某些支撑功能或依赖，必须说明必要性并等待用户确认
